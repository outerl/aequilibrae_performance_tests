...
cdef void init_heap_para(PriorityQueue* pqueue, size_t length, int num_threads) nogil:
    ...
    * int num_threads :  number of threads for the parallel job
    ...
    for i in prange(
        ...
        # pqueue.Elements[i].state = ElementState.NOT_IN_HEAP
        # pqueue.Elements[i].node_idx = length
...
cdef void _initialize_element(PriorityQueue* pqueue, size_t element_idx) nogil:
    ...
    * size_t element_idx : index of the element in the element array
    ...
    pqueue.Elements[element_idx].state = ElementState.NOT_IN_HEAP
...
cdef void insert(PriorityQueue* pqueue, size_t element_idx, DTYPE_t key) nogil:
    ...
    * its new key is smaller than DTYPE_INF
    ...
    pqueue.size += 1
    pqueue.Elements[element_idx].state = ElementState.IN_HEAP
...
cdef size_t extract_min(PriorityQueue* pqueue) nogil:
    ...
    * pqueue.size > 0
    ...
    pqueue.Elements[element_idx].state = ElementState.SCANNED
    pqueue.Elements[element_idx].node_idx = pqueue.length
